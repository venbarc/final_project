...
<html class="no-js" lang="">
  <head>
    ...
    <title>History Subject | Learning E.R.A</title>
                            ...
                            <?php
                                ...
                                if(isset($_GET['subject']) && isset($_GET['subject_token']))
                                {
                                    ...
                                    ?>
                                        <div class="row">
                                            <div class="col-md-1">
                                                <a href="class_prof.php?subject=<?php echo $subject?>&subject_token=<?php echo $subject_token ?>"><h2><i class="text-primary fa fa-chevron-circle-left"></i></h2></a>
                                                <br>
                                            </div>
                                            <div class="col-md-11">
                                                <h3 class="pb-50"> <span class="text-primary">Quizzes History</span> | <?php echo $subject ?></h3>
                                            </div>
                                        </div>
                                    <?php

                                    // get the quiz history in quiz_result db 
                                    // Check if the form is submitted
                                    if (isset($_POST['search_btn'])) 
                                    {
                                        $search_query = $_POST['search_query'];
                                        $sql_get_history = "select q.*, u.* from quiz_result q join users u on q.student_email = u.email
                                        where q.prof_email='$email' and q.subject_token = '$subject_token' 
                                        AND (
                                        q.student_email LIKE '%$search_query%' 
                                        OR u.fname LIKE '%$search_query%' OR u.lname LIKE '%$search_query%'
                                        OR q.score_percent LIKE '%$search_query%' 
                                        OR q.correct LIKE '%$search_query%'
                                        OR q.wrong LIKE '%$search_query%'
                                        OR q.unanswered LIKE '%$search_query%'
                                        OR q.score LIKE '%$search_query%'
                                        OR q.verdict LIKE '%$search_query%'
                                        OR q.date LIKE '%$search_query%' )
                                        ";
                                    }
                                    else
                                    {
